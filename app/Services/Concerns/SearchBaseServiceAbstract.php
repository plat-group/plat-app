...
abstract class SearchBaseServiceAbstract
{
    ...
    {
        $this->withLoad = $hasChange['withLoad'] ?? $this->withLoad;
    ...
    private function filterRequest()
    {
        ...
        if ($rawFilters) {
            Arr::forget($rawFilters, ['token', 'sort', 'fields', 'page', 'limit', 'download', 'withLoad']);

            // Parse raw filter from query request
            foreach ($rawFilters as $k => $v) {
                if (is_array($v)) {
                    $filterData['filter'][$k] = array_filter($v, function ($elm) {
